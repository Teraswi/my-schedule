...
if (isset($_POST['changes_edit'])) {
    $title__groop = $_POST['changes_edit']; // Название таблицы из POST-запроса
} elseif (isset($_GET['table'])) {
    $title__groop = $_GET['table']; // Название таблицы из GET-запроса
} else {
    $title__groop = null; // Ничего не выбрано
}

if ($title__groop) {
    $humanReadableTitle = '';

    if (!empty($title__groop)) {
        $tableWithoutPrefix = substr($title__groop, 3); // Убираем "ch_"
        ...
        if (count($tableParts) === 2) {
            ...
            $humanReadableTitle = "$datePart ($dayPart)";
        } else {
            $humanReadableTitle = $title__groop;
        }
    }
    echo "<h1 class='dekstop'>Вы редактируете изменения на $humanReadableTitle</h1>";
} else {
...
if(isset($_POST['changes_edit']))
   {
    ...
    <div class='buttom_column'>
        <button id='addColumnBtn_edit' class='add_column'>Добавить столбец</button>
        <button id='removeColumnBtn_edit' class='delete_column'>Удалить столбец</button>
    </div>
    ";
    $hidden_column = "0";
    ...
    $query_ch = "SELECT * FROM `$tableName`";
    $result_ch = mysqli_query($link, $query_ch);?>
    <section class='table changes'>
   ...
   $arr = [];
                ...
                <td class='choice_admin'>
                 <input type="hidden" name="hid" class="hid" value='<?php echo $currentID;?>'>
                  <div class='td_ob'>
                      ...
                      $subjectValue = '';
                      $officeValue = '';
                      if (!empty($data) && preg_match('/^(.+?)\s+([\d\w]+)\s*каб\.\s*$/iu', $data, $matches)) {
                          $subjectValue = trim($matches[1]); // Название предмета
                          $officeValue = trim($matches[2]); // Номер кабинета
                        }
                        /* 
                        Регулярное выражение значит следующее:
                        .+? — ленивый захват для названия предмета.
                        [\d\w]+ — номер кабинета (цифры и буквы).
                        \s* — игнорирует лишние пробелы.
                        i — регистронезависимый поиск.
                        u — поддержка Unicode (если используются русские буквы). */
                      ?>
    ...
    <div class='button_center'>
        <button class='admin_add' name='edit_changes' id='submitEditBtn'>Сохранить</button>
    </div>";
   }
   else
   {
